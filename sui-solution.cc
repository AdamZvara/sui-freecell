...
std::vector<SearchAction> getPath(MapType &paths, SearchSharedPtr final,
...
inline bool hasEmptySpot(const GameState &state) {
  auto free_cells_single_empty =
      std::find_if(state.free_cells.begin(), state.free_cells.end(),
                   [](const FreeCell &cell) {
                     return cell.canAccept(Card(Color::Heart, 1));
                   });
  auto stacks_single_empty = std::find_if(
      state.stacks.begin(), state.stacks.end(), [](const WorkStack &stack) {
        return stack.canAccept(Card(Color::Heart, 1));
      });
  return free_cells_single_empty != state.free_cells.end() ||
         stacks_single_empty != state.stacks.end();
}
...
// Using Heineman's staged deepening heuristic with comination of CardsOutOfHome
// heuristic Source:
...
std::vector<SearchAction> AStarSearch::solve(const SearchState &init_state) {
  ...
  while (!open.empty()) {
    ...
    for (SearchAction &action : current_state->actions()) {
      ...
      auto in_open = (in_map != paths.end())
        ? open.find(AStarOpenItem(new_state, 0, in_map->second.f_cost))
        : open.end();
